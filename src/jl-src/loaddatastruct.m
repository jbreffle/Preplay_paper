...
for i = 1:length(datafiles)
    if isempty(days)
        ...
        loadedDataStruct1 = load(fullfile(animaldir, datafiles(i).name));
        eval(['out = datavaradd(out,', ['loadedDataStruct1.', datatype],');']);
    else
        s = datafiles(i).name;
        ...
        if (isempty(fileday))|(ismember(fileday,days))
            ...
            %load([animaldir,datafiles(i).name]);
            loadedDataStruct2 = load(fullfile(animaldir, datafiles(i).name), datatype);

            eval([datatype, ' = loadedDataStruct2.', datatype, ';']);
